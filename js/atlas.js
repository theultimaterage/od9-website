...
(function () {
  ...
  /* Measurement lives in js/atlas-beacon.js — it owns no map state and every
     layer sends through it. Aliased here so the call sites read `ping(...)`,
     and so the map still runs if that file ever fails to load. */
  var ping = (window.AtlasBeacon && window.AtlasBeacon.ping) || function () {};
  ...
  /* The sky lives in js/atlas-cosmos.js: seeded starfield, the volume nebulae
     and their Flow plates, and the ambient meteors. It is the one layer that
     knows nothing about chapters or focus, which is why it separates cleanly.
     The names below are the same ones the draw loop has always called. */
  ...
  var drawStarGlow = COSMOS.starGlow, drawFlare = COSMOS.flare, drawMeteor = COSMOS.meteor;
