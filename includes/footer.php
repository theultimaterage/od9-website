...
// Plausible analytics (T-INF-WEB-ANALYTICS-001) — opt-in.
// config/analytics.php is gitignored + only present when activated.
$_od9_analytics_config = __DIR__ . '/../config/analytics.php';
if (file_exists($_od9_analytics_config)) {
    require_once $_od9_analytics_config;
}
?>
<style>
...
<li><a href="progress.php">Live Progress</a></li>
...
<?php
// Plausible analytics tracking pixel — only emits when config/analytics.php
// is present AND defines PLAUSIBLE_DOMAIN. Rendered as the LAST element of
// the body so it doesn't block initial paint.
if (defined('PLAUSIBLE_DOMAIN')):
    $_plausible_src = defined('PLAUSIBLE_SCRIPT_URL')
        ? PLAUSIBLE_SCRIPT_URL
        : 'https://plausible.io/js/script.js';
?>
<script defer data-domain="<?= htmlspecialchars(PLAUSIBLE_DOMAIN, ENT_QUOTES) ?>" src="<?= htmlspecialchars($_plausible_src, ENT_QUOTES) ?>"></script>
<?php endif; ?>
