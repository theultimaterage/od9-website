-- od9 tickets DB — STRUCTURE ONLY, generated from production.
-- GENERATED by regen_prod_schema.py
--
-- Ground truth for schema-usage-lint (.claude/schema-usage-lint-config.json).
-- DO NOT HAND-EDIT: the next regeneration overwrites it. Change the database
...
--
-- Read through the site's own bootstrap (SHOW CREATE TABLE over the app's
-- own PDO connection), so no database credential is needed on the account,
-- transported, or stored. Contains no data and no credentials.
--
-- AUTO_INCREMENT counters are stripped and objects are emitted in a fixed
-- order, so re-running against an unchanged database yields identical bytes.
-- View DEFINER accounts are normalised to CURRENT_USER.
...
CREATE TABLE `od9_discord_bot_config` (
  ...
  `config_data` longtext DEFAULT NULL CHECK (json_valid(`config_data`)),
...
CREATE TABLE `orders` (
  ...
  `customer_notes` mediumtext DEFAULT NULL,
...
CREATE TABLE `order_items` (
  ...
  `flagged_reason` mediumtext DEFAULT NULL,
...
CREATE TABLE `promo_codes` (
  ...
  `restricted_to_email` varchar(255) DEFAULT NULL COMMENT 'When set, only this customer email may redeem the code (loyalty/referral issued codes).',
  ...
  KEY `idx_restricted_to_email` (`restricted_to_email`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `ticket_templates` (
  ...
  `template_data` longtext DEFAULT NULL CHECK (json_valid(`template_data`)),
