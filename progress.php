...
// SOURCE OF TRUTH override: the bot's SQLite patreon_supporters (written directly by
// the Patreon webhook; tier_amount in dollars). The MySQL od9_members mirror's patreon
...
try {
    ...
    if (!defined('ENVIRONMENT') || ENVIRONMENT !== 'local') error_log('[progress.php] sqlite read failed: ' . $e->getMessage());
}
...
<div class="wrap">
  ...
  <?php
  // State of the Filter — the mission layer. OD9's receipts above; humanity's receipts here.
  require_once __DIR__ . '/includes/filter-scorecard-data.php';
  include __DIR__ . '/includes/filter-scorecard.php';
  ?>
  ...
  <p class="note">Updated <?= $updated ?> · Refresh for live numbers · <a href="founders.php">Founding ledger</a> · <a href="funding.php">The books — where every dollar goes</a></p>
