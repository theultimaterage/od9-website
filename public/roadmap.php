 ...
 *
 * Reads from od9_roadmap_triggers (mirror of bot's SQLite roadmap_triggers,
 * synced every 15 min by scripts/sync_to_mysql.py).
 *
 * URL params:
...
try {
    ...
    if ($detail_id !== '') {
        $rows = safe_query(
            $pdo,
            "SELECT * FROM od9_roadmap_triggers WHERE trigger_id = :id LIMIT 1",
            [':id' => $detail_id]
        );
        ...
        $active = safe_query(
            $pdo,
            "SELECT * FROM od9_roadmap_triggers WHERE status = 'Active'$domain_clause ORDER BY domain, trigger_id",
            $params
        );
        $approved = safe_query(
            $pdo,
            "SELECT * FROM od9_roadmap_triggers WHERE status = 'Approved'$domain_clause ORDER BY domain, trigger_id LIMIT 50",
            $params
        );
        $activated = safe_query(
            $pdo,
            "SELECT * FROM od9_roadmap_triggers WHERE status = 'Activated'$domain_clause ORDER BY activated_at DESC LIMIT 20",
            $params
        );
    }
    ...
    foreach (array_keys($STATUS_COLORS) as $status) {
        $stats[strtolower($status)] = safe_count(
            $pdo,
            "SELECT COUNT(*) FROM od9_roadmap_triggers WHERE status = :s",
            [':s' => $status]
        );
    }
    ...
    foreach (['seed', 'community', 'founder'] as $src) {
        $stats['from_' . $src] = safe_count(
            $pdo,
            "SELECT COUNT(*) FROM od9_roadmap_triggers WHERE source = :s",
            [':s' => $src]
        );
    }
} catch (Throwable $e) {
