...
if (!$email || !filter_var($email, FILTER_VALIDATE_EMAIL) || !$token || strlen($token) !== 32) {
    ...
    try {
        ...
        if (!$row) {
            ...
            $up->execute([$row['id']]);
            $state = 'verified';
            $message = "$email is confirmed. The first weekly email arrives Monday at 9 AM CT - and the QOTD goes live in the OD9 Discord daily at 12 PM CT if you want a head start.";
        }
...
<div class="card">
  ...
  <h1>$safeTitle</h1>
  <p>$safeMessage</p>
  <a class="btn" href="https://offda9.com">Back to offda9.com</a>
  <div class="links">
